...
int main(int argc, char *argv[]){
	...
	if(rank==0){	//actions of coordinator 
		for(int j=crd->getindex()+1;j<crd->input.size()+1;++j){
			...
			}/*todo else if(crd->get().at(0).compare("RETRIEVE")==0){
		...
		if(srv->leader_id==rank){
			while(srv->shutdown==false){	//actions of leader 
				...
				}/*todo else if(dummy_int[0]==RETRIEVE){
				...
				//map<int, h_entry*>::iterator it;
				//for ( it = srv->ld->htable.begin(); it != srv->ld->htable.end(); it++ )
				/*todo for(auto const &it : srv->ld->htable) 
				{
					if(it.second!=NULL){
		...
		while(!(clt->active_requests==0&&clt->shutdown==true) ){
			...
			/*}todoelse if(dummy_int[0]==RETRIEVE){
				MPI_Send(&dummy_int, 1, CUSTOM_ARRAY, clt->leader_id , TAG, MPI_COMM_WORLD);
				MPI_Recv(&dummy_int, 1, CUSTOM_ARRAY, clt->leader_id , TAG, MPI_COMM_WORLD, &status);
				if(dummy_int[0]!=RETRIEVE_OK && dummy_int[0]!=RETRIEVE_FAILED){
					cout<<"ERROR 8: "<<dummy_int[0]<<" was occured\n";
				}else if(dummy_int[0]==RETRIEVE_OK){
					bool exists=false;
